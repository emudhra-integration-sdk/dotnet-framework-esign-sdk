...
using emCastle.Asn1.Esf;
using emCastle.Math;
using emCastle.Tsp;
using emCastle.Asn1.Ocsp;
using emCastle.Asn1.X509;
using emCastle.Asn1.Ess;
using emCastle.Ocsp;
using emCastle.Asn1.Pkcs;
using emCastle.Asn1.Tsp;
using emCastle.Utilities;
namespace eSignLibrary.text.pdf.security {
    ...
    public class PdfPKCS7 {
        ...
        public PdfPKCS7(byte[] contentsKey, PdfName filterSubtype) {
            ...
            emCastle.Asn1.X509.X509Name issuer = emCastle.Asn1.X509.X509Name.GetInstance(issuerAndSerialNumber[0]);
            ...
            if (signerInfo[next] is Asn1TaggedObject) {
                ...
                for (int k = 0; k < sseq.Count; ++k) {
                    ...
                    if (idSeq2.Equals(SecurityIDs.ID_MESSAGE_DIGEST)) {
                        ...
                        Asn1Set setout = (Asn1Set)seq2[1];
                        Asn1Sequence seqout = (Asn1Sequence)setout[0];
                        SigningCertificate sv2 = emCastle.Asn1.Ess.SigningCertificate.GetInstance(seqout);
            ...
            if (next < signerInfo.Count && (signerInfo[next] is DerTaggedObject)) {
                ...
                emCastle.Asn1.Cms.AttributeTable attble = new emCastle.Asn1.Cms.AttributeTable(unat);
                emCastle.Asn1.Cms.Attribute ts = attble[PkcsObjectIdentifiers.IdAASignatureTimeStampToken];
                if (ts != null && ts.AttrValues.Count > 0) {
                    ...
                    emCastle.Asn1.Cms.ContentInfo contentInfo = emCastle.Asn1.Cms.ContentInfo.GetInstance(tokenSequence);
                    this.timeStampToken = new TimeStampToken(contentInfo);
                }
            }
            if (isTsp) {
                emCastle.Asn1.Cms.ContentInfo contentInfoTsp = emCastle.Asn1.Cms.ContentInfo.GetInstance(signedData);
        ...
        private DerSet GetAuthenticatedAttributeSet(byte[] secondDigest, byte[] ocsp, ICollection<byte[]> crlBytes, CryptoStandard sigtype) {
            ...
            if (signaturePolicyIdentifier != null) {
                attribute.Add(new emCastle.Asn1.Cms.Attribute(PkcsObjectIdentifiers.IdAAEtsSigPolicyID, new DerSet(signaturePolicyIdentifier)));
            }

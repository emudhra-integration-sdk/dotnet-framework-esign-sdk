...
using eSignLibrary.text.pdf.security;
...
namespace eSignLibrary.text.pdf
{
    ...
    public class PdfSignatureAppearance
    {
        ...
        /** Holds value of property DisableGreenTick. */
        private bool disableGreenTick;
        ...
        virtual public bool DisableGreenTick
        {
            get
            {
                return disableGreenTick;
            }
            set
            {
                disableGreenTick = value;
            }
        }
...
*/
        ...
        */
        ...
        private bool isMultipleAppreance = false;
        string fieldNamePrefix = "";
        private int[] pages = { 1 };
        virtual public int[] Pages
        {
            get
            {
                return pages;
        ...
        virtual public Rectangle[] PageRectangles
        {
            get
            {
        ...
        virtual public void SetVisibleSignature(Rectangle[] pageRects, int[] pages, string fieldName)
        {
            if (string.IsNullOrEmpty(fieldName))
            {
                throw new ArgumentException("fieldName is mandaory parameter");
            }
            //if (fieldName != null)
            //{
            //    if (fieldName.IndexOf('.') >= 0)
            ...
            isMultipleAppreance = true;
            if (pageRects == null)
                throw new ArgumentException(MessageLocalization.GetComposedMessage("field.pageRects.cannot.be.null"));
            if (pages == null)
                throw new ArgumentException(MessageLocalization.GetComposedMessage("field.pages.cannot.be.null"));
            if (pageRects.Length <= 0)
                throw new ArgumentException(MessageLocalization.GetComposedMessage("field.pageRects.length.must.be.greater.than.0"));
            if (pages.Length <= 0)
                throw new ArgumentException(MessageLocalization.GetComposedMessage("field.pages.length.must.be.greater.than.0"));
            this.pageRect = new Rectangle(pageRects[0]);
            this.pageRect.Normalize();
            rect = new Rectangle(this.pageRect.Width, this.pageRect.Height);
            this.pages = pages;
            newField = true;
            this.pageRectangles = new Rectangle[pageRects.Length];
            for (int i = 0; i < pageRects.Length; i++)
            {
                pageRectangles[i] = pageRects[i];
            }
        }
        ...
        */
        ...
        virtual public PdfTemplate GetAppearance()
        {
            ...
            if (app[2] == null)
            {
                ...
                else if (renderingMode == RenderingMode.GRAPHIC_AND_DESCRIPTION_RTL)
                {
                    if (signatureGraphic == null)
                    {
                        throw new InvalidOperationException(MessageLocalization.GetComposedMessage("a.signature.image.should.be.present.when.rendering.mode.is.graphic.only"));
                ...
                else if (renderingMode == RenderingMode.GRAPHIC)
                {
                    ...
                    Image im = Image.GetInstance(signatureGraphic);
                    im.ScaleAbsolute(signatureRect.Width, signatureRect.Height);
            ...
            if (frm == null)
            {
                ...
                if (DisableGreenTick)
                {
                    scale = Math.Min(rect.Width, rect.Height) * 0f;
                }
        ...
        */
        ...
        public Dictionary<PdfName, PdfLiteral> exclusionLocations;
        ...
        virtual public void PreClose(Dictionary<PdfName, int> exclusionSizes)
        {
            ...
            if (fieldExists)
            {
                ...
                page = isMultipleAppreance ? 1 : page;
            ...
            else
            {
                if (isMultipleAppreance)
                {
                    for (int i = 0; i < Pages.Length; i++)
                    {
                        ...
                        {
                            sigField.FieldName = fieldNamePrefix + "-Signature" + (i + 1);
                        }
                        else
                        {
                            sigField = PdfFormField.CreateSignature(writer);
                            int test = int.Parse(name.Substring(9));
                            sigField.FieldName = fieldNamePrefix + "-Signature" + (test + i);
                        }
                        sigField.Put(PdfName.V, refSig);
                        sigField.Flags = PdfAnnotation.FLAGS_PRINT | PdfAnnotation.FLAGS_LOCKED;
                        if (!IsInvisible())
                        {
                            if (PageRectangles.Length == 1)
                            {
                                sigField.SetWidget(PageRect, null);
                            }
                            else
                            {
                                sigField.SetWidget(PageRectangles[i], null);
                            }
                        }
                        else
                        {
                        ...
                        writer.AddAnnotation(sigField, Pages[i]);
                    }
                }
                else
                {

...
using System.IO;
using System.Text;
using eSignLibrary.awt.geom;
using eSignLibrary.text.exceptions;
using eSignLibrary.text.pdf.interfaces;
using eSignLibrary.text.pdf.intern;
        ...
        */
        ...
        virtual public void Rectangle(Rectangle rectangle) {
            ...
            if (rectangle.UseVariableBorders) {
                ...
                if (rectangle.BorderWidth != eSignLibrary.text.Rectangle.UNDEFINED) {
                ...
                if (rectangle.HasBorder(eSignLibrary.text.Rectangle.BOX)) {
                    this.Rectangle(x1, y1, x2 - x1, y2 - y1);
                }
                    // if the border isn't a rectangle, the different sides are added apart
                else {
                    if (rectangle.HasBorder(eSignLibrary.text.Rectangle.RIGHT_BORDER)) {
                        MoveTo(x2, y1);
                        LineTo(x2, y2);
                    }
                    if (rectangle.HasBorder(eSignLibrary.text.Rectangle.LEFT_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x1, y2);
                    }
                    if (rectangle.HasBorder(eSignLibrary.text.Rectangle.BOTTOM_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x2, y1);
                    }
                    if (rectangle.HasBorder(eSignLibrary.text.Rectangle.TOP_BORDER)) {
                        MoveTo(x1, y2);
                        LineTo(x2, y2);
                    }
                }

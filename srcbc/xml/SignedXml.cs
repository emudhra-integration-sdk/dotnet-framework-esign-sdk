...
namespace emCastle.Crypto.Xml
{
    public class SignedXml
    {
        ...
        private ISigner signatureDescription = null;
        ...
        public const string XmlDsigECDSASHA256Url = "http://www.w3.org/2001/04/xmldsig-more#ecsda-sha256";
        ...
        {
            ComputeSignature((byte[])null);
        }

        public void ComputeSignature(byte[] Signature)
        {
            ...
            signatureDescription = CryptoHelpers.CreateFromName<ISigner>(SignedInfo.SignatureMethod);
        ...
        {
            if (!(signatureDescription is DsaDigestSigner))
            {
                throw new NotImplementedException("Impliemented only on DsaDigestSigner");
            }
            return ((DsaDigestSigner)signatureDescription)?.GetSignedHash();
        }
        ...
        private bool CheckSignedInfo(AsymmetricKeyParameter key)
        {
            ...
            signatureDescription = CryptoHelpers.CreateFromName<ISigner>(SignatureMethod);
            if (signatureDescription == null)
            {
                throw new System.Security.Cryptography.CryptographicException(SR.Cryptography_Xml_SignatureDescriptionNotCreated);
            }
